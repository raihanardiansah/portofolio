...
export default function Projects() {
  ...
  useEffect(() => {
    document.title = `Projects | ${data.profile.name || 'Portfolio'}`;
  }, [data.profile.name]);
  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <div className="max-w-[760px] mx-auto px-5 py-8 sm:py-14">
        ...
        {filteredProjects.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {filteredProjects.map(p => (
              <div key={p.slug} className="flex flex-col rounded-2xl border border-zinc-200/80 dark:border-zinc-700/50 bg-white dark:bg-zinc-800/50 p-5 hover:border-black dark:hover:border-white/40 hover:shadow-xl hover:scale-[1.01] transition-all duration-200">
                ...
                <div className="mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-700/50 flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <Link to={`/projects/${p.slug}`} className="text-[11px] font-mono text-black dark:text-white hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">{getLoc(lang, 'View detail →', 'Lihat detail →', '查看详情 →', '詳細を見る →', '세부 정보 보기 →')}</Link>
                    {p.githubUrl && (
                      <a href={p.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1.5 text-[10px] font-mono px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-700/50 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors no-underline" aria-label={`${p.title} source code on GitHub`}>
                    ...
                    {p.liveUrl && (
                      <a href={p.liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center text-[10px] font-mono px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-700/50 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors no-underline" aria-label={`${p.title} live demo`}>Live ↗</a>
                    )}
                  </div>
                </div>
              </div>
        ...
        {/* FIX #13: Removed exposed /admin link from public footer */}
        <footer className="mt-20 pt-6 border-t border-zinc-200 dark:border-zinc-800 text-center">
          <Link to="/" className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 hover:text-zinc-600 dark:hover:text-zinc-400 transition-colors">← Home</Link>
        </footer>
      </div>
    </main>
  );
